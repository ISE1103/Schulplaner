...
function cycleSymptomChecks(selected=[]){return CYCLE_SYMPTOMS.map(s=>`<label class="cycle-symptom-check"><input type="checkbox" name="cycleSymptom" value="${s}" ${selected.includes(s)?"checked":""}><span>${s}</span></label>`).join("")}
function cycleDayLevel(day,value=""){return `<label>Beschwerden Tag ${day}<select name="day${day}Level"><option value="">– auswählen –</option>${CYCLE_LEVELS.map(x=>`<option value="${x}" ${x===value?"selected":""}>${x}</option>`).join("")}</select></label>`}
function openCycleEntry(id=null){
 if(!cycleUnlocked)return;
 const e=data.cycleEntries.find(x=>x.id===id)||{id:null,startDate:localYMD(new Date()),endDate:"",flow:"",symptoms:[],day1Level:"",day2Level:"",day3Level:"",mood:"",note:""};
 const selected=Array.isArray(e.symptoms)?e.symptoms:(e.symptoms?[e.symptoms]:[]);
 ...
 <fieldset class="cycle-symptom-fieldset"><legend>Typische Beschwerden <small>(mehrere gleichzeitig möglich)</small></legend><div class="cycle-symptom-grid">${cycleSymptomChecks(selected)}</div></fieldset>
 <div class="cycle-day-levels">${cycleDayLevel(1,e.day1Level||"")}${cycleDayLevel(2,e.day2Level||"")}${cycleDayLevel(3,e.day3Level||"")}</div>
 ...
 form=>{
  ...
  const o={id:e.id||uid(),startDate:f.get("startDate"),endDate:f.get("endDate")||"",flow:f.get("flow")||"",symptoms:f.getAll("cycleSymptom"),day1Level:f.get("day1Level")||"",day2Level:f.get("day2Level")||"",day3Level:f.get("day3Level")||"",mood:(f.get("mood")||"").trim(),note:(f.get("note")||"").trim(),updatedAt:new Date().toISOString()};
