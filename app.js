...
function ensureCycleData(){if(!Array.isArray(data.cycleEntries))data.cycleEntries=[];if(!data.cycleAccess)data.cycleAccess=null;}
function mergeCycleEntries(remoteEntries=[],localEntries=[]){
  const map=new Map();
  for(const e of [...remoteEntries,...localEntries]){
    if(!e?.id)continue;
    const prev=map.get(e.id);
    if(!prev||String(e.updatedAt||"")>=String(prev.updatedAt||""))map.set(e.id,e);
  }
  return [...map.values()];
}
function applyRemoteState(remote){
  const local=data||{};
  const cycleEntries=mergeCycleEntries(remote.cycleEntries||[],local.cycleEntries||[]);
  const ra=remote.cycleAccess,la=local.cycleAccess;
  const cycleAccess=!ra?la:!la?ra:(String(ra.updatedAt||"")>=String(la.updatedAt||"")?ra:la);
  data={subjects:remote.subjects||[],books:remote.books||[],tasks:remote.tasks||[],lessons:remote.lessons||[],familyEvents:remote.familyEvents||[],cycleEntries,cycleAccess:cycleAccess||null};
}
ensureCycleData();
ensureFamilyData();
...
async function cyclePasswordHash(value){
...
async function setupCyclePin(){
  const a=prompt("Gemeinsames Zyklus-Passwort (mindestens 6 Zeichen):");if(a===null)return;
  if(a.length<6){alert("Bitte mindestens 6 Zeichen verwenden.");return}
  const b=prompt("Passwort wiederholen:");if(a!==b){alert("Passwörter stimmen nicht überein.");return}
  ensureCycleData();data.cycleAccess={hash:await cyclePasswordHash(a),updatedAt:new Date().toISOString()};
  localStorage.removeItem("lara_cycle_pin_hash");saveData();
  alert("Gemeinsames Zyklus-Passwort gespeichert. Es wird mit dem Familienkonto synchronisiert.");
}
async function unlockCycle(value){
  ensureCycleData();const m=document.querySelector("#cyclePinMessage"),i=document.querySelector("#cyclePinInput");
  if(!data.cycleAccess?.hash){if(m)m.textContent="Bitte zuerst ein gemeinsames Zyklus-Passwort festlegen.";if(i)i.value="";return}
  if(await cyclePasswordHash(value)!==data.cycleAccess.hash){if(m)m.textContent="Passwort ist nicht korrekt.";if(i){i.value="";i.focus()}return}
  cycleUnlocked=true;if(m)m.textContent="";if(i){i.value="";i.blur()}
  document.querySelector("#cycleLocked")?.classList.add("hidden");document.querySelector("#cycleContent")?.classList.remove("hidden");renderCycle();
}
function cd(d){return new Date(d+"T12:00:00")} function cf(d){return d?cd(d).toLocaleDateString("de-DE"):"–"}
function renderCycle(){ensureCycleData();if(!cycleUnlocked)return;const a=[...data.cycleEntries].sort((x,y)=>x.startDate.localeCompare(y.startDate));const gaps=[];for(let i=1;i<a.length;i++){const n=Math.round((cd(a[i].startDate)-cd(a[i-1].startDate))/86400000);if(n>=15&&n<=60)gaps.push(n)}const avg=gaps.length?Math.round(gaps.reduce((x,y)=>x+y,0)/gaps.length):null,last=a.at(-1);let next=null;if(last&&avg){let d=cd(last.startDate);d.setDate(d.getDate()+avg);next=d.toISOString().slice(0,10)}document.querySelector("#cycleLastStart").textContent=last?cf(last.startDate):"–";document.querySelector("#cycleAverage").textContent=avg?avg+" Tage":"Noch nicht genug Daten";document.querySelector("#cycleNextEstimate").textContent=next?cf(next):"Noch nicht genug Daten";const l=document.querySelector("#cycleEntries");const r=[...a].reverse();l.innerHTML=r.length?r.map(e=>`<article class="cycle-entry" data-cycle-id="${e.id}"><div class="cycle-entry-top"><div class="cycle-entry-title">🌸 Periode</div><div class="cycle-entry-date">${cf(e.startDate)}${e.endDate?" – "+cf(e.endDate):""}</div></div><div class="cycle-entry-meta">${e.flow?`<span class="cycle-pill">Stärke: ${esc(e.flow)}</span>`:""}${(e.day1Symptoms?.length||e.day1Level)?`<span class="cycle-pill">Tag 1: ${esc([e.day1Level,...(e.day1Symptoms||[])].filter(Boolean).join(" · "))}</span>`:""}${(e.day2Symptoms?.length||e.day2Level)?`<span class="cycle-pill">Tag 2: ${esc([e.day2Level,...(e.day2Symptoms||[])].filter(Boolean).join(" · "))}</span>`:""}${(e.day3Symptoms?.length||e.day3Level)?`<span class="cycle-pill">Tag 3: ${esc([e.day3Level,...(e.day3Symptoms||[])].filter(Boolean).join(" · "))}</span>`:""}${e.mood?`<span class="cycle-pill">Stimmung: ${esc(e.mood)}</span>`:""}</div>${e.note?`<div class="cycle-entry-note">${esc(e.note)}</div>`:""}</article>`).join(""):'<div class="empty">Noch keine Einträge.</div>';l.querySelectorAll("[data-cycle-id]").forEach(x=>x.onclick=()=>openCycleEntry(x.dataset.cycleId));}


const CYCLE_SYMPTOMS=["Bauchkrämpfe","Rückenschmerzen","Kopfschmerzen","Brustspannen","Übelkeit","Blähungen","Müdigkeit","Schwindel","Durchfall","Verstopfung","Heißhunger","Schlafprobleme","Reizbarkeit","Stimmungsschwankungen"];
const CYCLE_LEVELS=["Keine","Leicht","Mittel","Stark"];

function cycleSymptomChecks(day,selected=[]){
  return CYCLE_SYMPTOMS.map(s=>`<label class="cycle-symptom-check"><input type="checkbox" name="day${day}Symptom" value="${s}" ${selected.includes(s)?"checked":""}><span class="cycle-symptom-label">${s}</span></label>`).join("");
}
function cycleDayBlock(day,selected=[],level=""){
  return `<fieldset class="cycle-day-fieldset"><legend>Tag ${day}</legend>
  <label>Beschwerdegrad<select name="day${day}Level"><option value="">– auswählen –</option>${CYCLE_LEVELS.map(x=>`<option value="${x}" ${x===level?"selected":""}>${x}</option>`).join("")}</select></label>
  <div class="cycle-symptom-grid">${cycleSymptomChecks(day,selected)}</div></fieldset>`;
}

function openCycleEntry(id=null){
  if(!cycleUnlocked)return;
  const e=data.cycleEntries.find(x=>x.id===id)||{id:null,startDate:localYMD(new Date()),endDate:"",flow:"",day1Symptoms:[],day2Symptoms:[],day3Symptoms:[],day1Level:"",day2Level:"",day3Level:"",mood:"",note:""};
  const oldSymptoms=Array.isArray(e.symptoms)?e.symptoms:(e.symptoms?[e.symptoms]:[]);
  const d1=Array.isArray(e.day1Symptoms)?e.day1Symptoms:oldSymptoms;
  const d2=Array.isArray(e.day2Symptoms)?e.day2Symptoms:[];
  const d3=Array.isArray(e.day3Symptoms)?e.day3Symptoms:[];
  openEditor(id?"Periodeneintrag bearbeiten":"Periode eintragen",`
    <div class="form-grid"><label>Beginn<input name="startDate" type="date" required value="${esc(e.startDate)}"></label><label>Ende<input name="endDate" type="date" value="${esc(e.endDate||"")}"></label></div>
    <label>Stärke der Blutung<select name="flow"><option value="">–</option>${["Leicht","Mittel","Stark"].map(x=>`<option ${x===e.flow?"selected":""}>${x}</option>`).join("")}</select></label>
    <div class="cycle-days-wrapper">${cycleDayBlock(1,d1,e.day1Level||"")}${cycleDayBlock(2,d2,e.day2Level||"")}${cycleDayBlock(3,d3,e.day3Level||"")}</div>
    <label>Stimmung<input name="mood" value="${esc(e.mood||"")}" placeholder="optional"></label>
    <label>Private Notiz<textarea name="note" rows="4" placeholder="optional">${esc(e.note||"")}</textarea></label>
  `,form=>{
    const f=new FormData(form);
    const o={
      id:e.id||uid(),startDate:f.get("startDate"),endDate:f.get("endDate")||"",flow:f.get("flow")||"",
      day1Symptoms:f.getAll("day1Symptom"),day2Symptoms:f.getAll("day2Symptom"),day3Symptoms:f.getAll("day3Symptom"),
      day1Level:f.get("day1Level")||"",day2Level:f.get("day2Level")||"",day3Level:f.get("day3Level")||"",
      mood:(f.get("mood")||"").trim(),note:(f.get("note")||"").trim(),updatedAt:new Date().toISOString()
    };
    if(o.endDate&&o.endDate<o.startDate){alert("Enddatum darf nicht vor Beginn liegen.");return}
    if(e.id)data.cycleEntries=data.cycleEntries.map(x=>x.id===e.id?o:x);else data.cycleEntries.push(o);
    saveData();renderCycle();
  },id?()=>{if(confirm("Eintrag löschen?")){data.cycleEntries=data.cycleEntries.filter(x=>x.id!==id);saveData();closeEditor();renderCycle();}}:null);
}
document.querySelector("#cycleUnlockForm")?.addEventListener("submit",async e=>{e.preventDefault();await unlockCycle(document.querySelector("#cyclePinInput").value)});
